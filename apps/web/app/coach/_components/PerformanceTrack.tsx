...
export function PerformanceTrack({ data }: { data: PerformanceData }) {
  ...
  const totalPlayers = data.totalLocationPlayers;
        ...
        <div className="flex bg-slate-200 rounded-lg p-1">
          <button
            ...
            className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${
              period === 'weekly' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Weekly
          </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-2">
          ...
          <span className="text-[22px] font-bold text-slate-900 leading-tight">
            {activePlayers}{totalPlayers !== undefined ? ` / ${totalPlayers}` : ''}
          </span>
