...
import { PastSessionCard, type PastSessionCardProps } from '../PastSessionCard';
import { Pagination } from '@/components/ui/Pagination';

export interface PastSessionItem extends PastSessionCardProps {
  id: string;
}

interface PastSessionsListViewProps {
  sessions: PastSessionItem[];
}

export function PastSessionsListView({ sessions }: PastSessionsListViewProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-slate-900">All Past Sessions</h1>
          <p className="text-xs text-slate-500 font-medium">
            {totalItems} conducted {totalItems === 1 ? 'session' : 'sessions'} recorded
          </p>
        </div>
      </div>
      ...
      {paginatedSessions.length > 0 ? (
        <div className="space-y-3">
          {paginatedSessions.map((session) => (
            <PastSessionCard key={session.id} {...session} />
          ))}
        </div>
      ) : (
        <div className="bg-white rounded-2xl p-8 border border-slate-100 text-center space-y-2">
          ...
          <p className="text-xs font-bold text-slate-500">No past sessions found</p>
        </div>
      )}
